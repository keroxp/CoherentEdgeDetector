...
class CreateDB : Closeable {
    val pool = Executors.newFixedThreadPool(10)
    override fun close() {
        pool.shutdown()
    }
    fun getMaxId(connection: Connection, colName: String): Int {
        val rs = connection.createStatement().executeQuery("SELECT MAX(id) FROM $colName;")
        rs.next()
    ...
    fun insert(images: List<File>, category: String) {
        ...
        var imageCount = AtomicInteger(getMaxId(connection, "Images"))
        var lineCount = AtomicInteger(getMaxId(connection, "CoherentLines"))
        ...
        try {
            ...
            images.forEach { file ->
                pool.execute {
                    var image_id = imageCount.andIncrement
                    ...
                    val outdir = File("out/$category/$name")
                    ...
                    Performance.calc("write cohl and minhash") {
                        for (label in labels) {
                            // Insert Line
                            val line_id = lineCount.andIncrement
                            lock.withLock {
                    ...
                    println("done category: $category, name: ${file.name}.")
                    latch.countDown()
                }
            }
            latch.await()
